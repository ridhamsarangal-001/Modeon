...
function LoginContent() {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // verifyOtpAction redirects on success — if we reach here it returned an error
    } catch (err: unknown) {
      // Next.js redirect throws — re-throw it so the router handles it
      if (
        err &&
        typeof err === "object" &&
        "digest" in err &&
      ...
      const result = err as { error?: string } | null;
